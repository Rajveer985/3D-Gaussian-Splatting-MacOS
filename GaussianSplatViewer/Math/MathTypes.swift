...
struct GaussianGPUData {
    ...
    init(from s: GaussianSplat) {
        ...
        var c = s.shCoefficients
        while c.count < 48 { c.append(0) }

        // Determine highest SH degree with non-zero higher-order coefficients
        var deg: UInt32 = 0
...
// MARK: - Splat settings  (matches SplatSettings in Metal exactly)
// Passed to GPU every frame so all UI sliders take effect immediately.
...
extension float4 {
    static func fromAxisAngle(axis: float3, angle: Float) -> float4 {
        let h = angle * 0.5, s = sin(h)
        return float4(axis.x*s, axis.y*s, axis.z*s, cos(h))
    }
    var normalized: float4 {
        let l = simd_length(self); return l > 0 ? self/l : float4(0,0,0,1)
    }
...
extension float4x4 {
    ...
    static func lookAt(eye: float3, center: float3, up: float3) -> float4x4 {
        let f = normalize(eye-center), r = normalize(cross(up,f)), u = cross(f,r)
        return float4x4(
    ...
    // Legacy alias so Camera.swift compiles unchanged
    static func perspective(fovRadians: Float, aspect: Float, nearZ: Float, farZ: Float) -> float4x4 {
        perspective(fovY: fovRadians, aspect: aspect, near: nearZ, far: farZ)
    }
}

// MARK: - Covariance (used by Scene.swift)

extension GaussianSplat {
    func computeCovariance() -> (Float,Float,Float,Float,Float,Float) {
        let S = float3x3(diagonal: scale), R = rotation.toRotationMatrix
        let RS = R*S, Σ = RS*RS.transpose
        return (Σ[0][0],Σ[0][1],Σ[0][2],Σ[1][1],Σ[1][2],Σ[2][2])
    }
}

// MARK: - Legacy stubs so Scene.swift compiles unchanged

struct ProjectedGaussian {
    var depth: Float = 0; var index: UInt32 = 0; var uv: float2 = .zero
    var conic: float3 = .zero; var color: float3 = .zero
    var opacity: Float = 0; var radius: Float = 0
}
struct RasterConfig {
