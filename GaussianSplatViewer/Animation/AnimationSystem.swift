...
final class AnimationSystem: ObservableObject, AnimationSystemProtocol {
    ...
    func scrub(to frame: Int) {
        ...
        // Only apply interpolated state if there are actually keyframes set
        guard hasAnyKeyframes else { return }
        isAnimating = true
    ...
    func applyInterpolatedState(at frame: Int) {
        guard let camera else { return }
        // Only apply if keyframes exist — otherwise leave camera untouched
        guard hasAnyKeyframes else { return }
        InterpolationEngine.evaluate(at: Double(frame), store: store).apply(to: camera)
    }

    var hasAnyKeyframes: Bool {
        AnimatableProperty.allCases.contains { !store.keyframes(for: $0).isEmpty }
    }
